"""Ana akşam / sabah iş mantığı - v24 + otomatik paper trading"""
import json
import logging
import time
...
def aksam(cfg: dict):
    ...
    # Önce mevcut sanal pozisyonları güncelle (stop/hedef)
    paper, paper_mesajlar = paper_guncelle(paper, GUNCEL, bugun, cfg, trades_path)
    satlar.extend(paper_mesajlar)

    # Yeni al listesini sanal olarak al (zaten açık olanları alma)
    paper_acik = {p["hisse"] for p in paper.get("pozisyonlar", [])}
    for s in top:
        if s["hisse"] in paper_acik:
            continue
        paper = paper_al(
            paper, s["hisse"], s["fiyat"], s.get("poz_yuzde", 15),
            s["stop"], s["hedef"], bugun.strftime("%Y-%m-%d")
        )
    paper_kaydet(paper_path, paper)
    # ------------------------------------
    ...
    if top:
        ...
        mesaj += ("\n⭐TV AL | 🔊hacim | 📰haber | 🏭sektör momentum\n"
                  f"💡 Sanal portföy otomatik alım yapıyor\n")
    else:
        mesaj += "\n😴 Liste boş.\n"
...
def sabah(cfg: dict):
    ...
    if gecmis:
        ...
        mesaj += "— Yok\n"
    mesaj += "\n💡 Sanal portföy otomatik yönetiliyor."
    print(mesaj)
    telegram_gonder(cfg["telegram"]["token"], cfg["telegram"]["chat_id"], mesaj)
